...
//             ref={bikeRef}
//             style={{
//               transform: isJumping ? "translateY(-120px)" : "translateY(0)",backgroundImage: `url(${getBikeImage()})`,
...
export default function BikeGame() {
  ...
  useEffect(() => {
    if (gameOver) return;
    const speed = window.innerWidth < 480 ? 7 : window.innerWidth < 768 ? 9 : window.innerWidth < 1024 ? 11 : 12;
    const interval = setInterval(() => {
      setObstacleX((prev) => {
        if (prev < -100) {
          const isBird = Math.random() < 0.3;
  ...
  useEffect(() => {
      ...
      if (bike && obs) {
        ...
        if (hit) {
          ...
          const current = {
            ...
            jumps,
            maxHeight: Number(maxHeight.toFixed(2)),
            minHeight: Number(minHeight === Infinity ? 0 : minHeight.toFixed(2)),
            timestamp: new Date().toLocaleString(),
          };
          const updatedHistory = [current, ...scoreHistory].slice(0, 10);
          setScoreHistory(updatedHistory);
          localStorage.setItem("bikeScoreHistory", JSON.stringify(updatedHistory));
          try {
            const ws = XLSX.utils.json_to_sheet(updatedHistory);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Bike Scores");
            ...
            saveAs(blob, `Bike_Score_History_${new Date().toISOString().split("T")[0]}.xlsx`);
          } catch (error) {
            console.error("❌ Error saving Excel file:", error);
            const jsonData = JSON.stringify(updatedHistory, null, 2);
            const jsonBlob = new Blob([jsonData], { type: "application/json" });
            saveAs(jsonBlob, `Bike_Score_History_${new Date().toISOString().split("T")[0]}.json`);
          }
          setHighScore((prevHigh) => {
            const better = {
              distance: Math.max(prevHigh.distance, current.distance),
    ...
    return () => clearInterval(interval);
  }, [gameOver, distance, jumps, maxHeight, minHeight, scoreHistory, obstacleType]);
